...
import { ArrowLeft, CalendarCheck, ChevronLeft, ChevronRight, Menu, Stethoscope } from "lucide-react";
...
export default function PatientPortal({ initialStep = "home" }: Props) {
  ...
  const calendar = () => {
    ...
    return <article className="panel-card"><div className="calendar-header"><button type="button" className="icon-button" aria-label="Mes anterior" disabled={month <= clinicDate().slice(0, 7)} onClick={() => changeMonth(-1)}><ChevronLeft /></button><h2>{start.toLocaleDateString("es-CO", { month: "long", year: "numeric" })}</h2><button type="button" className="icon-button" aria-label="Mes siguiente" disabled={month >= maxDay.slice(0, 7)} onClick={() => changeMonth(1)}><ChevronRight /></button></div><div className="weekdays">{["Lun", "Mar", "Mié", "Jue", "Vie", "Sáb", "Dom"].map(d => <span key={d}>{d}</span>)}</div><div className="calendar-grid">{Array.from({ length: offset }, (_, i) => <span key={`blank-${i}`} />)}{Array.from({ length: count }, (_, i) => { const day = `${month}-${String(i + 1).padStart(2, "0")}`; const weekday = new Date(year, m - 1, i + 1).getDay(); const disabled = day < clinicDate() || day > maxDay || [0, 6].includes(weekday); return <button key={day} type="button" aria-label={day} aria-pressed={date === day} disabled={disabled} className={`date-button ${disabled ? "disabled" : ""} ${date === day ? "selected" : ""}`} onClick={() => { setDate(day); setTime(""); setError(""); }}>{i + 1}</button>; })}</div><p className="legend">Lunes a viernes · Hora de Colombia · Hasta 180 días</p></article>;
  };

  return <div className="clinic-app"><header className="topbar"><button className="logo-button" onClick={() => navigate("home")} aria-label="Inicio"><Image src="/logo-clinica-belen.png" alt="Clínica Belén" width={180} height={64} className="logo" priority /></button><nav className={`topnav ${menu ? "menu-open" : ""}`} aria-label="Principal">{([["home", "Inicio"], ["lookup", "Citas"], ["specialty", "Especialidades"], ["contact", "Contacto"]] as [PortalStep, string][]).map(([target, label]) => <button key={target} className={`nav-link ${step === target ? "active" : ""}`} disabled={!ready || busy} onClick={() => navigate(target)}>{label}</button>)}</nav><button className="primary-pill desktop-only" disabled={!ready || busy} onClick={() => navigate(patient ? "account" : "identify")}>{patient ? "Mi cuenta" : "Ingresar"}</button><button className="icon-button mobile-only" aria-label="Abrir menú" aria-expanded={menu} onClick={() => setMenu(!menu)}><Menu /></button></header>
    <main className="app-canvas"><section className="content-panel"><div aria-live="polite">{!ready && <p role="status">Cargando portal…</p>}{busy && <p role="status">Guardando…</p>}{notice && <p className="notice" role="status">{notice}</p>}</div>{error && <div className="error-banner" role="alert">{error}{!patient && ready && <button className="text-button" onClick={() => navigate("identify")}>Ingresar</button>}</div>}
      ...
      {step === "contact" && <article className="panel-card"><h2>Clínica Belén de Fusagasugá</h2><p>Consulte con recepción los canales oficiales de atención, las políticas de privacidad y los derechos del paciente.</p><p>La agenda de este portal ofrece consultas de lunes a viernes, de 8:00 a 12:00 y de 14:00 a 17:00, en hora de Colombia.</p><button className="primary-button" onClick={() => navigate("identify")}>Agendar cita</button></article>}
      </div>}</fieldset>}
